...
/// Rounds strategy probabilities to multiples of `step_percent`.
///
/// The `strategy` layout is `[action0_hands..., action1_hands..., ...]`.
/// Each hand's action probabilities are rounded to multiples of `step_percent%`
/// while preserving a sum of 1.0 (when the original sum is positive).
#[inline]
pub(crate) fn round_strategy_to_step(
    strategy: &mut [f32],
    num_actions: usize,
    step_percent: u8,
) {
    if step_percent == 0 || num_actions == 0 || strategy.is_empty() {
        return;
    }

    if step_percent > 100 || 100 % step_percent as u16 != 0 {
        return;
    }

    let buckets = (100 / step_percent as u16) as i32;
    let row_size = strategy.len() / num_actions;
    if row_size == 0 {
        return;
    }

    let mut bases = vec![0i32; num_actions];
    let mut remainders = vec![0f32; num_actions];
    let mut indices = (0..num_actions).collect::<Vec<_>>();

    for hand in 0..row_size {
        let mut sum = 0.0f32;
        for action in 0..num_actions {
            let idx = action * row_size + hand;
            let mut p = strategy[idx];
        ...
        if sum <= 0.0 {
            for action in 0..num_actions {
                strategy[action * row_size + hand] = 0.0;
            }
            continue;
        }
        ...
        for action in 0..num_actions {
            let idx = action * row_size + hand;
            let p = strategy[idx] / sum;
            let raw = p * buckets as f32;
            let base = raw.floor();
            bases[action] = base as i32;
            remainders[action] = raw - base;
            base_sum += bases[action];
        }

        let mut remaining = buckets - base_sum;
        if remaining > 0 {
            indices.sort_by(|&a, &b| {
                remainders[b]
            ...
            let mut i = 0usize;
            while remaining < 0 && i < indices.len() {
                let idx = indices[i];
                if bases[idx] > 0 {
                    bases[idx] -= 1;
                    remaining += 1;
        ...
        let denom = buckets as f32;
        for action in 0..num_actions {
            strategy[action * row_size + hand] = (bases[action].max(0) as f32) / denom;
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::round_strategy_to_step;

    #[test]
    fn test_round_strategy_step_25() {
        // 3 actions, 2 hands
        let num_actions = 3;
        ...
        round_strategy_to_step(&mut strategy, num_actions, 25);

        // Each hand should sum to 1.0 and be multiples of 0.25
        let row_size = strategy.len() / num_actions;
        for hand in 0..row_size {
            let mut sum = 0.0;
            for action in 0..num_actions {
                let p = strategy[action * row_size + hand];
                let scaled = (p * 100.0).round() as i32;
