...
impl PostFlopGame {
    ...
    /// Sets rounding step size in percent.
    ///
    /// - `0` disables rounding.
    /// - Otherwise must be a factor of 100 (e.g., 1, 2, 4, 5, 10, 20, 25, 50, 100).
    #[inline]
    pub fn set_rounding_step_percent(&mut self, step_percent: u8) -> Result<(), String> {
        if step_percent == 0 {
            self.rounding_step_percent = 0;
            return Ok(());
        }

        if step_percent > 100 || 100 % step_percent as u16 != 0 {
            return Err(format!(
                "Rounding step size must be a factor of 100: {step_percent}"
            ));
        }
    ...
    #[inline]
    pub fn round_strategies_in_place(&mut self) {
        let step_percent = self.rounding_step_percent;
        if step_percent == 0 || self.state < State::MemoryAllocated {
            return;
        }
        ...
        for node in &self.node_arena {
            ...
            let mut normalized = if self.is_compression_enabled {
            ...
            apply_locking_strategy(&mut normalized, locking);

            round_strategy_to_step(&mut normalized, num_actions, step_percent);

            if self.is_compression_enabled {
                let scale = encode_unsigned_slice(node.strategy_compressed_mut(), &normalized);
                node.set_strategy_scale(scale);
            } else {
                node.strategy_mut().copy_from_slice(&normalized);
            }
        }
    }
